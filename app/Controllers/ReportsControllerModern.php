...
class ReportsControllerModern extends Controller
{
    ...
    private function treasuryPayload(string $from, string $to, float $currentRate): array
    {
        ...
        return [
            ...
            'headers' => ['Cuenta', 'Tipo', 'Metodo', 'Moneda', 'Saldo original', 'Equiv. VES', 'Equiv. USD', 'Entradas', 'Salidas', 'Neto periodo'],
            'widths' => [40, 18, 24, 14, 22, 22, 22, 20, 20, 22],
            'alignments' => ['L', 'L', 'L', 'L', 'R', 'R', 'R', 'R', 'R', 'R'],
            'rows' => array_map(fn (array $row): array => [
                (string) ($row['account_name'] ?? ''),
                account_type_label((string) ($row['account_type'] ?? 'bank')),
